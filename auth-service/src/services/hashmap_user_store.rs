...
mod tests {
    ...
    async fn test_validate_user() {
        ...
        let valid_credentials = hash
            .validate_credentials(valid_user.clone(), valid_password.clone())
            .await;
        assert_eq!(valid_credentials, Ok(true));

        let invalid_credentials = hash
            .validate_credentials(valid_user.clone(), invalid_password.clone())
            .await;
        assert_eq!(invalid_credentials, Err(UserStoreError::InvalidCredentials));

        let non_existent_user = hash
            .validate_credentials(invalid_user.clone(), valid_password.clone())
            .await;
        assert_eq!(non_existent_user, Err(UserStoreError::UserNotFound));
    }
}
